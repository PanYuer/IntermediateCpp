...
void selectionSort(const char *list[], int size)
{
    ...
    for (curr = 0; curr < last; curr++)
    {
        ...
        for (walk = curr + 1; walk < size; walk++)
            // if (list[walk] < list[small]) // Error: the statement is comparing pointers, the address of constant char
            if (strcmp(list[walk], list[small]) < 0) // compare the strings that the pointers point to
                small = walk;
